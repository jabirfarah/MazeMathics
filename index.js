...
db.prepare('CREATE TABLE IF NOT EXISTS something (world TEXT)').run();
...
const world = JSON.parse(db.prepare('SELECT world FROM something').pluck().get() ?? '[]');
...
if (!world.length) {
	...
	world[SPAWN_X][SPAWN_Y] =0;
}

const WORLD_SIZE_X = world.length;
const WORLD_SIZE_Y = world[0].length;
const PLAYER_VIEW_RADIUS_X = 7;
const PLAYER_VIEW_RADIUS_Y = 7;
		...
		try {
			...
			switch (message.action) {
				...
				case 'place':
					...
					updateAllPlayers();
					db.prepare('UPDATE something SET world = ?').run(JSON.stringify(world));
					// console.log('placed');
			}
		} catch (error) {
			console.log(error);
